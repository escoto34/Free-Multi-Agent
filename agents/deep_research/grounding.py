...
def run_grounding(
    ...
    """Run the Grounding step using the provider/model configured in
    config/model_router.yaml (``deep_research.grounding``) — currently
    Cohere command-a-plus-05-2026 with native ``documents`` grounding.

    The model is asked for plain cited prose (not a self-authored JSON
    wrapper); the GroundedReport object is constructed in Python afterwards.
    ...
    cfg = get_agent_config("deep_research", "grounding")
